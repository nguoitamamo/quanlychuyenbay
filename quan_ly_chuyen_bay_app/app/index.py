...
import quan_ly_chuyen_bay_app.app.utils as utils
import math
import quan_ly_chuyen_bay_app.app.vnpay as vnpay
            ...
            if request.method == "POST":
                ...
                if mat_khau == xac_nhan:
                    nguoi_dung = dao.tao_nguoi_dung_moi(ten_nguoi_dung=ten_nguoi_dung,
                                                        CCCD=CCCD,
                                                        tai_khoan=tai_khoan, mat_khau=mat_khau, hoat_dong=True,
                                                        gmail=gmail,
                    ...
                    if nguoi_dung:
                        ...
                        for so_dem in range(sl_so_dien_thoai):
                            ...
                            if len(so_dien_thoai) == 10:
                                so_dien_thoai = dao.tao_so_dien_thoai(id_nguoi_dung=nguoi_dung.id_nguoi_dung,
...
@app.route("/captcha")
def captcha():
    captcha_text = utils.generate_captcha_text()
    session['captcha'] = captcha_text
    img_io = utils.generate_captcha_image(captcha_text)
    ...
    if not (current_user.is_authenticated and current_user.user_role == UserRole.NHAN_VIEN):
        ...
        if request.method == 'POST':
            ...
            if captcha_input != session.get('captcha', ''):
                err = "Sai mã CAPTCHA! Vui lòng thử lại."
                return render_template("KhachHang/DangNhap.html", err=err)
...
def xem_thong_tin_nguoi_dung():
    if current_user.is_authenticated and current_user.user_role == UserRole.KHACH_HANG:
        ...
        if sl_so_dien_thoai and int(sl_so_dien_thoai) <= 3 and int(sl_so_dien_thoai) >= 1:
                ...
                if mat_khau == xac_nhan:
                    if dao.sua_nguoi_dung(current_user.id_nguoi_dung
                        ...
                        if kiem_tra:
                            ...
                            for so_dem in range(sl_so_dien_thoai):
                                kiem_tra = dao.tao_so_dien_thoai(id_nguoi_dung=current_user.id_nguoi_dung,
                                                                 so_dien_thoai=request.form.get(
                                                                     "so_dien_thoai_" + str(so_dem)))
                                if kiem_tra:
                                    ds_so_dien_thoai = dao.lay_ds_so_dien_thoai_theo_id_nguoi_dung(
            ...
            return render_template("KhachHang/ThongTinNguoiDung/XemThongTinNguoiDung.html", mes=mes,
                                   ds_so_dien_thoai=ds_so_dien_thoai, sl_so_dien_thoai=sl_so_dien_thoai)
        else:
            return redirect("/XemThongTinNguoiDung?sl_so_dien_thoai=" + str(
                dao.lay_so_luong_so_dien_thoai_theo_id_nguoi_dung(id_nguoi_dung=current_user.id_nguoi_dung)))
    else:
        return redirect("/DangNhap")
...
def cap_nhat_khach_hang():
    if current_user.is_authenticated and current_user.user_role == UserRole.NHAN_VIEN:
        ...
        if sl_so_dien_thoai and int(sl_so_dien_thoai) <= 3 and int(sl_so_dien_thoai) >= 1:
            ...
            if request.method == 'POST':
                ...
                if kiem_tra:
                    ...
                    if nguoi_dung:
                        ...
                        dao.xoa_so_dien_thoai(id_nguoi_dung=nguoi_dung.id_nguoi_dung)
                        for so_dem in range(sl_so_dien_thoai):
                            kiem_tra = dao.tao_so_dien_thoai(id_nguoi_dung=nguoi_dung.id_nguoi_dung,
                                                             so_dien_thoai=request.form.get(
                                                                 "so_dien_thoai_" + str(so_dem)))
                            if kiem_tra:
            ...
            return render_template("/NhanVien/QuanLyKhachHang/CapNhatKhachHang.html", sl_so_dien_thoai=sl_so_dien_thoai,
                                   khach_hang=khach_hang, ds_so_dien_thoai=ds_so_dien_thoai, err=err)
        else:
            return redirect("/NhanVien/QuanLyKhachHang/CapNhatKhachHang?id_khach_hang=" + str(
                id_khach_hang) + "&sl_so_dien_thoai=" + str(
                dao.lay_so_luong_so_dien_thoai_theo_id_nguoi_dung(id_nguoi_dung=id_khach_hang)))
    else:
        return redirect("/NhanVien")
            ...
            if request.method == "POST":
                ...
                if kiem_tra:
                    nguoi_dung = dao.tao_nguoi_dung_moi(ten_nguoi_dung=ten_nguoi_dung, gmail=gmail, CCCD=CCCD,
                                                        mat_khau=None, hoat_dong=None, tai_khoan=tai_khoan,
                                                        anh_dai_dien=None)
                    if nguoi_dung:
                        dao.xoa_so_dien_thoai(id_nguoi_dung=nguoi_dung.id_nguoi_dung)
                        for so_dem in range(sl_so_dien_thoai):
                    ...
                    err = "số điện thoại không hợp lệ"
            return render_template("/NhanVien/QuanLyKhachHang/TaoKhachHang.html", err=err,
                                   sl_so_dien_thoai=sl_so_dien_thoai)
        else:
        ...
        if sl_so_dien_thoai and int(sl_so_dien_thoai) <= 3 and int(sl_so_dien_thoai) >= 1:
            ...
            if request.method == 'POST':
                ...
                if mat_khau == xac_nhan:
                    ...
                    mes = "lỗi mật khẩu và xác nhận không trùng khớp"
            return render_template("NhanVien/XemThongTinNguoiDung.html", mes=mes, ds_so_dien_thoai=ds_so_dien_thoai,
                                   sl_so_dien_thoai=sl_so_dien_thoai)
        else:
            return redirect("/NhanVien/XemThongTinNguoiDung?sl_so_dien_thoai=" + str(
                dao.lay_so_luong_so_dien_thoai_theo_id_nguoi_dung(id_nguoi_dung=current_user.id_nguoi_dung)))
    else:
        return redirect("/DangNhap")
...
if __name__ == "__main__":
    from quan_ly_chuyen_bay_app.app.admin import *
